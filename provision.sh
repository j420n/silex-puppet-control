...
install_activemq() {
    #Install ActiveMQ 5.14.1 manually as 5.6.0 from debian repository is broken.
    command -v activemq >/dev/null 2>&1 || {
        echo >&2 "Installing ActiveMQ...";
        cd /tmp
        if [ ! -f apache-activemq-5.14.1-bin.tar.gz ]
          then
            wget http://mirrors.ukfast.co.uk/sites/ftp.apache.org/activemq/5.14.1/apache-activemq-5.14.1-bin.tar.gz
            tar -zxvf apache-activemq-5.14.1-bin.tar.gz
        fi
        if [ -d apache-activemq-5.14.1-bin ]
          then
            mv apache-activemq-5.14.1 /usr/local/
            ln -sf /usr/local/apache-activemq-5.14.1 /usr/local/activemq
            chown -R activemq: /usr/local/apache-activemq-5.14.1/
